### GAS evaluation  
Here, we use PBMC 10K as an example to show how the Gene activity score(GAS) calculating methods are evaluated.  
...
GASs[[4]]<-readRDS("pbmc_mae_sample.rds")
...
# lst contain 4 GASs and gene_expression all on same set of genes
```
**Start evaluation**  
...
Exp=lst$Gene_exp
GAS_list=lst$GASs
GAS_list[[3]]<-GAS_list1[[3]]*10^5 ### as cicero GAS numbers are all small scale, we amplify to force them to have similar magnitude  with other GASs
```
### 1. Pearson correlation coefficient (PCC) between GAS and gene expression
```r
cor_tab<-matrix(0,4,dim(Exp)[1])
cor_tab_hv<-matrix(0,4,2000)  ### we also provide a selection of only highly variable genes correlation 
for(i in 1:4){
...
GAS_list[[5]]<-Exp ## Here we RNA is evaluated together with GASs as a reference

ASW_GAS=matrix(0,length(unique(true_label$SS)),5)
for (i in 1:5){
ASW_GAS[,i]<-PCA_ASW(GAS_list[[i]],true_label$SS)
 }
colnames(ASW_GAS)<-c("Signac","liger","cicero","maestro","scRNA")
```
