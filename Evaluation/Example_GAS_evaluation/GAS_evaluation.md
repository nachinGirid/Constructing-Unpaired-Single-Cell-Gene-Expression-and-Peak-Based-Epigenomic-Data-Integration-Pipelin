...
source("~/Evaluation/Functions/Gene_activity_score/Func_GAS_eval.r")
lst<-unify_genes_cells(Gene_Exp,GASs)
...
for(i in 1:3){
    ...
    cor_tab_hv[i,]<-correlation[[2]]#we aslo checked highly variable gene separately, not shown in manuscript
}
```
...
GAS_list[[4]]<-Exp ## Here RNA is evaluated together with GASs as a reference
