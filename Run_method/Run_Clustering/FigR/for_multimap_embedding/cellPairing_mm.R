...
plotPairs <- function(
  ...
  ggplot(umap.df, aes(x = UMAP1, y = UMAP2)) +
    ...
    geom_line(
      data = d,
      aes(x = UMAP1, y = UMAP2, group = pair),
      alpha = 0.4,
      color = "darkgray"
    ) +
    theme_void() +
...
smartUpSample <- function(mat1, mat2, seed = 123, dist.use = "pearson"){
  ...
  pairing.greedy <- data.frame(
    "bigger"  = names(cor.max),
    "smaller" = as.character(cor.max),
  ...
  for (i in small.cells) {
    numBiggerpairs <- c(
      numBiggerpairs,
      sum(pairing.greedy$smaller %in% i)
    )
  }
  ...
  numBiggerpairssmoothed <- unlist(lapply(
    1:length(numBiggerpairs),
    function(i){
  ...
  small.upCells <- sample(
    small.cells,
    size     = length(big.cells),
    replace  = TRUE,
    prob     = samplingDensity
  )
  ...
  cat(
    "% smaller dataset coverage up-on up-sampling: ",
    round((length(unique(small.upCells)) / length(small.cells)) * 100, 2),
    "\n"
  )
...
# Author: Nachin
umap_knn_graph <- function(
  x,                    # already-embedded MultiMap / other DR
  umap_dim         = 5, # ignored
  umap_n_nerighbors = 30,
  k               = 5,
  seed
){
  library(FNN)
  ...
  # Use x directly as the embedding (no uwot::umap)
  umap_embedding <- as.matrix(x)

  # KNN on this embedding
  all_knn <- get.knn(umap_embedding, k = k)
  n_data  <- nrow(x)

  knn_adj_mtx <- matrix(0, nrow = n_data, ncol = n_data)
  for (i in 1:n_data) {
    knn_adj_mtx[i, all_knn$nn.index[i, ]] <- 1
  }
...
all.unique <- function(x) length(x) == length(unique(x))


# This function takes in the output of the fullmatch function and sort the results into a list of ATAC-RNA pairs
# Author: Yan Hu
get_pair_list <- function(
  cell_matches,  # The output object of fullmatch or pairmatch in the package optmatch
...
cell_pairing <- function(
  ATACpcs,          # Input ATAC single cell PCs obtained from Running CCA dimensionality reduction. Shape: (n_ATAC_cells, n_PCs)
  RNApcs,           # Input RNA single cell PCs obtained from Running CCA dimensionality reduction. Shape: (n_RNA_cells, n_PCs)
  mode             = "geodesic", # Mode of pairing. One of "geodesic", or "greedy",
  tol              = 0.0001,     # See optmatch::fullmatch details
  search_range     = 0.2,        # search_range * total number of cells = size of knn.
  ...
  min_subgraph_size = 50,        # Minimum number of cells (ATAC/RNA each) needed for pairing in a given subgraph.
  cca_umap_df      = NULL,       # Optional umap of all (ATAC + RNA cells) to visualize UMAP of cells from each chunk being paired
  ...
  if (mode == "geodesic") {
    ...
    shortest_paths <- distances(knn_graph)

    # Find connected subgraphs
    subgraphs <- components(knn_graph)
    ...
    for (subgraph_ind in unique(subgraphs$membership)) {
      ...
      subgraph_geodist <- shortest_paths[
        match(rownames(subgraph_ATAC_pcs), rownames(all_pcs)),
      ...
      subgraph_size <- dim(subgraph_geodist)[2]
      ...
      print(paste(
        ...
        dim(geodist_knn)[1], "ATAC and",
        dim(geodist_knn)[1], " RNA cells"
      ))
...
chunk_CCA <- function(CCA_1, CCA_2, chunkSize = NULL, seed = 123){
  ...
  for (i in 1:frac) {
    ...
    smallCellsBag <- rownames(smaller)
    ...
    # Update what has been sampled already
  ...
  if (remain > 0) {
    chunkList[[i + 1]] <- list(
      bigger[rownames(bigger)[!rownames(bigger) %in% bigCells], ],
...
#' @return data.frame of ATAC/RNA barcodes
...
pairCells <- function(
  ...
  # Additional parameters passed to cell_pairing function (see below)
  ...
  if (nrow(ATAC) != nrow(RNA)) {
    ...
    if (all(rownames(chunkList[[1]][[1]]) %in% rownames(ATAC) &
            !rownames(chunkList[[1]][[2]]) %in% rownames(ATAC))) {
      ATAC.i <- 1
      RNA.i  <- 2
    } else {
      ATAC.i <- 2
    ...
    for (i in 1:length(chunkList)) {
      ...
      pairList[[i]] <- cell_pairing(
        ATACpcs          = chunkList[[i]][[ATAC.i]],
    ...
    print(min_subgraph_size)

    pair.df <- cell_pairing(
      ATACpcs          = ATAC,
      RNApcs           = RNA,
      mode             = "geodesic",
      tol              = 0.0001,
